...
double ManhattanDistanceCalculator::calculateDistance(const TreeNode &nodeA,
                                               const TreeNode &nodeB) const {
    double dx = fabs(nodeA[0]-nodeB[0]);
    double dy = fabs(nodeA[1]-nodeB[1]);
    return  dx + dy;
}
...
ManhattanDistanceCalculator::calculateVerticalDistance(const TreeNode &root,
    ...
    return fabs(root[dim] - target[dim]);
}
